...
const bnToEnNumberMap = {
 ...
 const convertToBanglaNumber = (text) =>
  text.split("").map((c) => enToBnNumberMap[c] || c).join("");

const convertToEnglishNumber = (text) =>
  text.split("").map((c) => bnToEnNumberMap[c] || c).join("");

  const handleNameInput = (e) => {
    let value = e.target.value;
    // Allow only Bangla letters & space
    value = value.replace(/[^\u0980-\u09FF\s]/g, "");
    setFormData((prev) => ({ ...prev, name: value }));
  };

  // -------------------------
  // DOB handler → smart auto 0 / slash
  // -------------------------
 const handleDOBInput = (e) => {
  let raw = e.target.value;

  // 1️⃣ Convert Bangla digits to English for processing
  raw = convertToEnglishNumber(raw);

  // 2️⃣ Keep only digits and slashes
  raw = raw.replace(/[^0-9/]/g, "");
  raw = raw.replace(/\/+$/g, ""); // remove trailing slashes

  // 3️⃣ Split by slash
  let parts = raw.split("/");

  // 4️⃣ Normalize day/month (auto-leading 0 if single digit but allow 2 digits)
  parts = parts.map((p, i) => {
    if (i < 2) { // day/month
      if (p.length === 1 && !raw.endsWith("/")) p = "0" + p; // only prepend 0 if not already typing second digit
      if (p.length > 2) p = p.slice(0, 2); // max 2 digits
    }
    if (i === 2 && p.length > 4) p = p.slice(0, 4); // year max 4 digits
    return p;
  });

  // 5️⃣ Reconstruct final value
  let finalValue = "";
  if (parts.length === 1) {
    finalValue = parts[0];
    if (parts[0].length === 2 && !raw.endsWith("/")) finalValue += "/"; // auto-slash after day
  } else if (parts.length === 2) {
    finalValue = parts[0] + "/" + parts[1];
    if (parts[1].length === 2 && !raw.endsWith("/")) finalValue += "/"; // auto-slash after month
  } else {
    finalValue = parts.join("/");
  }

  // 6️⃣ Convert to Bangla digits for display
